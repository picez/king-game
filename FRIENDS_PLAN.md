...
| **26.1** ✅ **DONE** | **Friend invite Join is actionable** — the invite toast's Join used to only PREFILL the Join sheet (menu) or reload to `/?room=` (in-game) — both left the user with "nothing happened". Now a **"Join room"** tap actually JOINS: at the menu it calls `joinRoom(code)` (the same server JOIN as the manual button); in another room it **confirms** ("Leave your current room to join this invite?") then routes the code through the menu (`App.onJoinInvite` → `StartMenu.initialInviteCode` → direct join, having dropped the stale resumable); in the SAME room it just dismisses. Falls back to prefilling the Join sheet only on a real conflict (no name / a different saved resumable). The `?room=` deep-link still prefills (unchanged); the payload is still a room code only. |

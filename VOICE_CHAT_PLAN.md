...
| **25.8** ✅ **DONE** | **Voice real-flow debug + audio-sink reliability** — the Lobby debug block now shows the raw **ICE state** (new/checking/connected/completed/failed/disconnected, via `oniceconnectionstatechange`) and a richer **Audio** state (**playing / blocked / no-track**). Remote audio sinks are now **attached to the DOM (hidden)** rather than detached (`new Audio()` alone is unreliable on some mobile browsers) and `play()` **clears the blocked flag on success**. Still opt-in, STUN-by-default; same-LAN two-tab is the audio regression check, cross-network needs TURN (25.6). |

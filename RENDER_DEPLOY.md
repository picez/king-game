...
> checklist to confirm health / 7 games / rooms / stats / avatars / social / security.
...
# {"status":"ok","version":"0.4.8","commit":"…","uptime":42,"db":"enabled",
...
## Android app (TWA) readiness (Stage 33.2 scaffold — no store submission yet)

The plan is an **Android Trusted Web Activity** wrapping this deployed PWA (see
[`MOBILE_APP_PLAN.md`](MOBILE_APP_PLAN.md)). The **config scaffold** is committed at `android-twa/`
(Bubblewrap `twa-manifest.json` → host `king-game-cqgd.onrender.com`, package `com.cardmajlis.app`); the
native Gradle project/APK/AAB is generated by the owner (`bubblewrap init`) — see
[`android-twa/README.md`](android-twa/README.md).

> **Full-screen TWA needs a custom domain + a real Asset Links file.** A build whose signing cert isn't in
> a served `assetlinks.json` opens as a **Custom Tab (URL bar)**, not full-screen. The **ordered production
> runbook** (custom domain → OAuth → PWA verify → signed AAB → **Play App-Signing SHA-256** → real
> `assetlinks.json` → verification) is **[`MOBILE_APP_PLAN.md`](MOBILE_APP_PLAN.md) §9**. Ops prerequisites,
> when 33.3+ builds it:

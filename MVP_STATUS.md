...
| **51** (Syrian 51) | 2–4 | **LOCAL PLAYABLE — experimental; online redaction-ready (Stage 30.4).** Pure core + local UI (1 human + bots); Local picker enabled ("Experimental"), **Host/online still disabled** (`supportsOnline:false` → CREATE_ROOM rejects it). serverCore can drive/redact/persist 51 (readiness only). Rummy-style meld/discard; 51-point opening; elimination at 510. No online/stats yet. [`51_RULES.md`](51_RULES.md) · [`51_PLAN.md`](51_PLAN.md) |
...
enables it (flagged "Experimental"); the **Host/online** picker keeps it **disabled**. As of
Stage 30.4 the server core is **online redaction-ready** — `serverCore` can build, authorise,
redact, auto-advance and persist a 51 game internally (proven by tests), but hosting stays OFF
(`supportsOnline:false` → `CREATE_ROOM` rejects a 51 room) until the 30.5 online release. Stats
(30.6) and favorites are still not wired. It is a rummy-style

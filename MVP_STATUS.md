...
| **Tarneeb** | 4 | Fixed 2×2 partnerships, bid 3–13, target 41 (solo cutthroat **pure core** exists but hidden — no UI/online/stats, [`TARNEEB_SOLO_PLAN.md`](TARNEEB_SOLO_PLAN.md)); [`TARNEEB_RULES.md`](TARNEEB_RULES.md) |
...
- **Solo / individual modes (Stage 28.0–28.2).** Deberc's two released modes are **named
  ...
  *is* the mode. **Stage 28.2** fixed the product gap the owner reported (Deberc still felt
  ...
  online, no stats, lobby/team UI unchanged — and exercised only by `solo.test.ts`. Released Tarneeb
  **pairs** is byte-for-byte unchanged (`soloGuard.test.ts`). Spec + status in
  [`TARNEEB_SOLO_PLAN.md`](TARNEEB_SOLO_PLAN.md); original A/B/C analysis in
  [`SOLO_VARIANTS_PLAN.md`](SOLO_VARIANTS_PLAN.md). **Next: Stage 28.2** — local-only solo setup +
  playable prototype.

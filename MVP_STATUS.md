...
patch packaging Stages 33.8–33.10 (the owner-run Android **TWA** build path hardened with a build-log
template + a read-only build-log **triage helper** + the production **Asset Links + custom-domain** runbook);
on the `v0.4.2` mobile app readiness — a **docs + PWA** patch packaging Stages 33.0–33.6 (Android TWA
...
| **Poker** | 2–6 | **RELEASED — local + online + stats + favorite + achievement (Stage 37.4, Unreleased build).** No-Limit Texas Hold'em pure core + shared UI; **both Local and Host pickers enabled**. Local pass-and-play has a **per-hand handover screen** so hole cards stay private. Online is server-authoritative (create/join/start, bots, seeded next-hand advance, **per-viewer hole-card redaction — deck order + burns never leave the server**, reconnect). **Score-only stats + leaderboard** under `game_type='poker'` (no DB migration) with a Profile sub-tab; **favoritable** + 4 poker achievements + counts toward All-Rounder (now **7 games**); own PNG chip emblem. side/split pots, best-5-of-7. The shared room cap rose **5 → 6**. **Bankroll economy (Stage 37.7, Unreleased):** chip **wallet** + daily 1M claim (migration 0010, `poker_wallets`/`poker_ledger`); **online = bankroll** — 8 host-picked stakes (buy-in 100 BB, server-derived) + optional blind growth, **atomic all-or-nothing buy-in escrow** with idempotent payout/refund, **authenticated-humans-only** (no bots); **local free-play** keeps a configurable starting stack (no wallet). New **oval table UI** (2–6 seats, RTL-stable) + **showdown review** (exact winning-five highlight, ~7 s server-paced) + hand-rankings **Help** + collapsible log. [`POKER_RULES.md`](POKER_RULES.md) · [`POKER_PLAN.md`](POKER_PLAN.md) |
...
- **Stats & leaderboard (Stage 5 / 5.1 / 5.2)**: when a `DATABASE_URL` is set,
  ...
  §3 Stage 5/5.1/5.2.
- **Achievements / badges foundation (Stage 16.0; +1 in 28.6; +15 in 32.1)**: a Profile **Achievements**
  tab (4th tab, next to Account / My stats / Leaderboard) showing **29** badges in a
  ...
  **Soloist (Stage 28.6 — win a Tarneeb Solo; reads the separate `tarneeb-solo`
  ...
  - **Expansion shipped (Stage 32.1)** — [`ACHIEVEMENTS_PLAN.md`](ACHIEVEMENTS_PLAN.md) §4: **+15 new
    badges (14→29)**, still **derived from existing stats only** — a basic win badge for every game that
    lacked one (Deberc/Tarneeb/Preferans/51), play-N / win-N depth badges, a **min-sample** contract-
    success skill badge (≥70% over ≥10 decided), and global milestones (Six-Game Regular = played each,
    Champion's Circle = 25 total wins). Adds an `uncommon` rarity tier. **No DB migration, no new stats
    field, no tutorial-progress/social/private data**; All-Rounder and the aggregates are **unchanged**.
    Deferred ideas (streaks, exact-bid, Deberc solo/pairs split, 51 quick-open/joker-trade, Tutorial
    Graduate) each still wait for a new stat field.

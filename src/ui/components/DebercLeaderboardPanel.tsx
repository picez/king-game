...
import { type DebercLeaderboardEntry, type Loadable } from '../../net/statsApi';

interface Props {
  result: Loadable<DebercLeaderboardEntry[]> | null;
  loading: boolean;
}
...
/**
 * Compact, mobile-first Deberc leaderboard (DEBERC-STATS-4). Public data only:
 * rank, avatar, name, games, wins, win rate + jackpot count. Two-line rows (no
 ...
 * the client).
 */
export default function DebercLeaderboardPanel({ result, loading }: Props) {
  const { t } = useI18n();
        ...
        <li key={i} className={`lb-row ${r.self ? 'lb-row--me' : ''}`}>
          ...
          <span className="lb-main">
            ...
            <span className="lb-meta">
              ...
              {' · '}{t('stats.jackpotShort')} {r.jackpotCount}
            </span>
          </span>
        </li>
      ))}

...
import { type FiftyOneLeaderboardEntry, type Loadable } from '../../net/statsApi';

interface Props {
  result: Loadable<FiftyOneLeaderboardEntry[]> | null;
  loading: boolean;
}
...
/**
 * Compact, mobile-first 51 leaderboard (FIFTYONE-STATS-4). Public data only: rank,
 * avatar, name, games, wins, win rate + best penalty (lower is better). Two-line
 * rows (no wide table) so it never overflows on 360/390 and stays correct under
 * RTL. The caller's own row is highlighted via the server `self` flag (no user id
 * reaches the client). Mirrors PreferansLeaderboardPanel.
 */
export default function FiftyOneLeaderboardPanel({ result, loading }: Props) {
  const { t } = useI18n();
        ...
        <li key={i} className={`lb-row ${r.self ? 'lb-row--me' : ''}`}>
          ...
          <span className="lb-main">
            ...
            <span className="lb-meta">
              ...
              {' · '}{t('stats.bestPenaltyShort')} {r.bestPenalty == null ? '—' : r.bestPenalty}
            </span>
          </span>
        </li>
      ))}

...
import { type PokerLeaderboardEntry, type Loadable } from '../../net/statsApi';

interface Props {
  result: Loadable<PokerLeaderboardEntry[]> | null;
  loading: boolean;
}
...
/**
 * Compact, mobile-first poker leaderboard (POKER-STATS-4). Public data only: rank,
 * avatar, name, games, wins, win rate + biggest pot. Two-line rows (no wide table)
 * so it never overflows on 360/390 and stays correct under RTL. The caller's own
 * row is highlighted via the server `self` flag (no user id reaches the client).
 * Mirrors FiftyOneLeaderboardPanel.
 */
export default function PokerLeaderboardPanel({ result, loading }: Props) {
  const { t } = useI18n();
        ...
        <li key={i} className={`lb-row ${r.self ? 'lb-row--me' : ''}`}>
          ...
          <span className="lb-main">
            ...
            <span className="lb-meta">
              ...
              {' · '}{t('poker.stats.biggestPot')} {r.biggestPot}
            </span>
          </span>
        </li>
      ))}

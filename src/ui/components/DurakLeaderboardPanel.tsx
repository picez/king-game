...
import { type DurakLeaderboardEntry, type Loadable } from '../../net/statsApi';

interface Props {
  result: Loadable<DurakLeaderboardEntry[]> | null;
  loading: boolean;
}
...
/**
 * Compact, mobile-first Durak leaderboard (DURAK-4). Public data only: rank,
 * avatar, name, games, wins, win rate + fool count. Two-line rows (no wide table)
 ...
 */
export default function DurakLeaderboardPanel({ result, loading }: Props) {
  const { t } = useI18n();

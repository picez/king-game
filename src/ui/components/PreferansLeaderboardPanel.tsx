...
import { type PreferansLeaderboardEntry, type Loadable } from '../../net/statsApi';

interface Props {
  result: Loadable<PreferansLeaderboardEntry[]> | null;
  loading: boolean;
}
...
/**
 * Compact, mobile-first Preferans leaderboard (PREFERANS-STATS-4). Public data
 * only: rank, avatar, name, games, wins, win rate + contract success. Two-line
 ...
 * reaches the client). Mirrors TarneebLeaderboardPanel.
 */
export default function PreferansLeaderboardPanel({ result, loading }: Props) {
  const { t } = useI18n();

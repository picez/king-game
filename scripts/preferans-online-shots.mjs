// ---------------------------------------------------------------------------
// Online Preferans board QA screenshots (Stage 19.5, experimental) via headless
// Chrome + CDP. Spins up a REAL server (port 3001, fast bots) so the client's
// default WS URL connects; drives the HOST UI: create Preferans room → capture the
// lobby at 1/3 → add 2 bots (3/3) → start → online bidding / talon / trick play.
// Captures at 360/390 and reports horizontal overflow.
//
//   node scripts/preferans-online-shots.mjs <clientUrl> <outDir>
//
...
const OUT = process.argv[3] || '.shots/preferans-online';
const CHROME = 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe';
const CDP_PORT = 9227;
const SRV_PORT = 3001;
const DATA = '.data-shots-pref';
...
// One human action: take talon, bury 2 (select then confirm), declare min, win the
// auction with the MAX bid, play the first legal card. (No Next-hand online.)
const HUMAN_STEP = `(()=>{
...
async function run() {
  ...
  try {
    ...
    for (const cfg of CONFIGS) {
      ...
      await sleep(1200);
      // Host online → pick Preferans → create.
      await cdp.evaluate(CLICKSEL('.tile', 1));            // Host online room
      await sleep(400);
      await cdp.evaluate(CLICKSEL('.game-picker .select-menu__trigger'));
      await sleep(300);
      await shot(cdp, `${cfg.tag}-0-host-picker`);
      await cdp.evaluate(`(()=>{const o=[...document.querySelectorAll('.select-menu__option')].find(x=>x.textContent.includes('Preferans')||x.textContent.includes('Преферанс')||x.textContent.includes('🎩'));if(o){o.click();return true}return false})()`);
      await sleep(300);
      await cdp.evaluate(CLICK('Create room', 'Створити'));
      await sleep(900);
      // Lobby at 1/3 (host only), then add 2 bots → 3/3.
      await shot(cdp, `${cfg.tag}-1-lobby-1of3`);
      for (let i = 0; i < 2; i++) { await cdp.evaluate(CLICK('Add bot', 'Додати бота', '🤖')); await sleep(300); }
      await shot(cdp, `${cfg.tag}-2-lobby-3of3`);
      await cdp.evaluate(CLICK('Start game', 'Почати гру', 'Start'));
      await sleep(900);

      const grabbed = { bid: false, talon: false, play: false };
      for (let i = 0; i < 200; i++) {
        if (!grabbed.bid && await cdp.evaluate(`!!document.querySelector('.preferans-bidbar')`)) { await shot(cdp, `${cfg.tag}-3-bidding`); grabbed.bid = true; }
        if (!grabbed.talon && await cdp.evaluate(`!!document.querySelector('.preferans-talonbar, .preferans-discardbar, .preferans-declarebar')`)) { await shot(cdp, `${cfg.tag}-4-talon`); grabbed.talon = true; }
        if (!grabbed.play && await cdp.evaluate(`!!document.querySelector('.preferans-play .card')`)) { await shot(cdp, `${cfg.tag}-5-playing`); grabbed.play = true; break; }
...
run().catch((e) => { console.error('preferans-online-shots crashed:', e); process.exit(1); });
